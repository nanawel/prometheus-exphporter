...
export PHP_OPTS=-d open_basedir=
...
install: checkreq
	which composer && composer install \
		|| { wget https://getcomposer.org/composer-stable.phar -O composer.phar && chmod +x composer.phar && php $(PHP_OPTS) ./composer.phar -n install; }
...
.PHONY: start
start:
	php $(PHP_OPTS) -S $(LISTEN_ADDR) index.php

.PHONY: startd
startd:
	nohup php $(PHP_OPTS) -S $(LISTEN_ADDR) index.php >> data/server.log 2>&1 &

.PHONY: kill
kill:
	kill $$(ps a | grep 'php $(PHP_OPTS) -S $(LISTEN_ADDR)' | grep -v grep | awk '{ print $$1 }')