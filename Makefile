...
install: checkreq
	which composer && composer install \
		|| { wget https://getcomposer.org/composer-stable.phar -O composer.phar && chmod +x composer.phar && php -c php.ini ./composer.phar -n install; }
...
.PHONY: start
start:
	php -S $(LISTEN_ADDR) -c php.ini index.php

.PHONY: startd
startd:
	nohup php -S $(LISTEN_ADDR) -c php.ini index.php >> data/server.log 2>&1 &
